...
const IngresoUsuarios = () => {
  ...
  const handleInputChange = async (event) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        throw new Error(
          "No se pudo completar el inicio de sesión. Verifica tus datos e inténtalo nuevamente."
        );
      }
      const responseData = await respuesta.json();
      setUserRole(responseData.role);
      setError(null);
      setLoggedIn(true); // Cambia el estado loggedIn después de validar el inicio de sesión
  ...
  const handleLoginError = (error) => {
    ...
    setError(
      "Hubo un error en el inicio de sesión. Verifica tus credenciales."
    );
  };
